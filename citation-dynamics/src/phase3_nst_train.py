...
Model configuration (defaults):
    ...
    J_encoder   = 3    (encoder depth; upstream uses 10 for 768-dim SPECTER input)
    J_snowflake = 3
    J_partialorder = 3
...
from phase3_nst_adapter import FEATURE_DIM, load_aps_data  # noqa: E402


# ---------------------------------------------------------------------------
# Stabilised model subclass
# ---------------------------------------------------------------------------

class APSNeuralSpacetime(NeuralSpacetime):
    """NeuralSpacetime with L2-normalised encoder outputs.

    Two fixes for our 4-dim structural features vs the 768-dim SPECTER inputs
    the upstream model was designed for:

    1. tanh on encoder output: bounds each element to (-1, 1) with a strictly
       positive Jacobian diagonal.  F.normalize would work but its Jacobian has
       an exact zero eigenvalue (in the direction of the vector), which combines
       with POActivation's |x|^0.5 gradient singularity at x=0 to give inf*0=NaN
       in the backward pass before gradient clipping can intervene.

    2. POActivation clamp (in layers.py): x_abs.clamp(min=1e-6) prevents the
       gradient singularity d/dx[|x|^s] → ∞ as x→0 when s<1.  Both fixes are
       needed: tanh prevents zero-eigenvalue collisions; the clamp catches any
       residual near-zero values inside PartialOrder's LeakyReLU path.
    """

    def forward(self, x: torch.Tensor, y: torch.Tensor):
        # tanh bounds each element to (-1, 1) with a strictly positive Jacobian —
        # unlike F.normalize whose Jacobian has an exact zero eigenvalue that
        # combines with POActivation's x^0.5 singularity to give inf*0=NaN.
        encoded_x = torch.tanh(self.encoder(x))
        encoded_y = torch.tanh(self.encoder(y))

        distance = self.snowflake(encoded_x[:, :self.D], encoded_y[:, :self.D])
        po_x = self.partialorder(encoded_x[:, self.D:])
        po_y = self.partialorder(encoded_y[:, self.D:])
        return distance, po_x, po_y

    @torch.no_grad()
    def encode_normalized(self, x: torch.Tensor) -> torch.Tensor:
        """Return tanh-bounded encoder output (consistent with training forward pass)."""
...
def train(
    ...
    J_encoder:        int   = 3,
    J_snowflake:      int   = 3,
    J_partialorder:   int   = 3,
    device: torch.device = torch.device("cpu"),
) -> nn.Module:
    """Train APSNeuralSpacetime and return the trained model.
    ...
    """
    model = APSNeuralSpacetime(
        N  = feature_dim,
        D  = space_dim,
        T  = time_dim,
        J_encoder       = J_encoder,
        J_snowflake     = J_snowflake,
        J_partialorder  = J_partialorder,
    ).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch == 1 or epoch % display_epoch == 0:
            ...
            print(
                ...
                f"{dist_str} | order_correct={correct:.3f} | gnorm={avg_gnorm:.3f}"
            )
...
def export_embeddings(
    ...
    for start in range(0, N, batch_size):
        ...
        # Use encode_normalized to match the normalisation applied during training
        if hasattr(model, "encode_normalized"):
            enc = model.encode_normalized(feat_t[start:end])
        else:
...
def main() -> None:
    ...
    parser.add_argument("--J_encoder",    type=int,   default=3,
                        help="Encoder depth (default 3; original uses 10 for 768-dim input)")
    parser.add_argument("--J_snowflake",  type=int,   default=3)
    parser.add_argument("--J_partialorder", type=int, default=3)
    ...
    # Sanity check
    if not torch.isfinite(data_x).all() or not torch.isfinite(data_y).all():
        raise ValueError("data_x or data_y contains NaN/Inf — rebuild with --force_rebuild")
    ...
    model = train(
        ...
        J_snowflake      = args.J_snowflake,
        J_partialorder   = args.J_partialorder,
        device           = device,
    )

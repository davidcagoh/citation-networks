function [C_pareto, B_pareto, authorName_pareto, doi_pareto, pubDate_pareto] = pareto_ordering_intra(C_sub, B_sub, authorName_sub, doi_sub, pubDate_sub, paretoParameter)
    ...
    out_degrees = sum(C_sub, 1)'; % Column vector of out-degrees
    ...
    B_pareto = B_sub(new_order, :);
    authorName_pareto = authorName_sub(new_order);
    ...
    figure;
    spy(C_pareto);
    title('Spy Plot of Adjacency Matrix (Pareto Split Ordering)');
    xlabel('Citing Articles (Out-degree)');
    ylabel('Cited Articles (In-degree)');

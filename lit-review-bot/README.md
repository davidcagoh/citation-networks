...
Legacy shell folder grouping the former LitDiscover engine, the eval infrastructure that validates it, and the
research-run project folders that use it (or the manual pipeline that's currently being explored
as a candidate replacement). Literature Workbench is now the active functional
system; these independently versioned assets remain available as predecessor
implementations, adapters, and evidence.
...
|---|---|---|
| [`litdiscover/`](litdiscover/) | Predecessor discovery engine — queue-driven citation-graph traversal, LLM screening, extraction, synthesis. `pip install litdiscover`. | own repo: `github.com/davidcagoh/litdiscover` (private) |
...
The active functional system is [`../literature-workbench/`](../literature-workbench/).

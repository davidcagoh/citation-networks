...
export function WorkbenchApp({ api }: { api: WorkbenchApi }) {
  ...
  async function handleDiscovery() {
    ...
    try {
      ...
      const discovery = await api.runDiscovery(project.id, prompt.trim(), 20, discoveryRoutes);
  ...
  return (
    <>
      ...
      {audit && <section className={styles.reviewToolbar} aria-label="Corpus checkpoint audit">
        ...
        <span>{audit.screening.selected}/{audit.screening.total} selected · {audit.source_text.selected_with_usable_text}/{audit.source_text.selected_total} with usable text · {audit.source_text.selected_with_full_text} full text{audit.source_text.selected_abstract_only > 0 ? ` · ${audit.source_text.selected_abstract_only} abstract only` : ""}</span>
        {(audit.routes.summaries ?? []).length > 0 && <span>Route yield · {(audit.routes.summaries ?? []).map((summary) => `${summary.route.replace("_search", "")}: ${summary.unique_papers} (${summary.new_unique_papers} new, ${summary.overlap_papers} overlap)`).join(" · ")}</span>}
        {(audit.routes.summaries ?? []).some((summary) => summary.queries.length > 0) && <span>Queries · {(audit.routes.summaries ?? []).filter((summary) => summary.queries.length > 0).map((summary) => `${summary.route.replace("_search", "")}: ${summary.queries.join(" | ")}`).join(" · ")}</span>}
        ...
        {audit.network?.expansion_attempts > 0 && <span>Citation network · {audit.network.edges} edges · {audit.network.papers_discovered} papers discovered · {audit.network.empty_expansions} empty expansions</span>}

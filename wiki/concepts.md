...
**Where it could plug in:** Downstream of the LitDiscover graph export (`<slug>_graph.h5` / `<slug>_papers.json`) or the Synthesis subgraph — as an alternative to Leiden in `synthesis/synthesis.md`'s (Q-SYNTH section) clustering comparison, not a replacement for Zeitgeist's Leiden-based power-law analysis (which needs a graph partition, not a soft mixture).
...
- **Reusing the Time Curves pipeline (already specced in `zeitgeist/nst-timecurves-comparison.md` for the full 709K-node Zeitgeist corpus, archived as out-of-scope there) on a single LitDiscover project's subgraph.** NST → SG-t-SNE → Time Curves was built and proxy-verified (`phase4_timecurves.py`) for full-corpus phase detection; the new h5 export makes it trivial to point the same pipeline at one traversal's output instead, without the special-cased DOI extraction `synthesis/synthesis.md` (Q-SYNTH section) currently requires for K17-RGC.
- **Directly composes with the Synthesis pipeline:** the h5 export is already in the format `synthesis/synthesis.md`'s (Q-SYNTH section) Leiden/BlueRed and NST/SG-t-SNE/UMAP comparisons expect, so any LitDiscover project's output becomes a synthesis test case for free, not just the manually-extracted K17-RGC case.

...
**Context:** the three-method lineage comparison (`../synthesis/background/lineages/`) found that citation-tracing alone
...
(see `../synthesis/background/lineages/similarity-cluster.md`). The traversal/discovery core is not behind SOTA — nothing in
...
**Discovery/recall figures (Q1-Q5, Q9-Q10, Q15) moved to `../discovery/open-questions.md`**
(2026-07-14, part of promoting Discovery to its own study) — Q1 in particular is now flagged
stale against `../discovery/roadmap.md` §4.0's end-to-end redesign, not just "resolved." Only the
non-discovery item stays here:
...
**Discovery/screening-specific items (Groq backend fallback, the prefilter idea's history, the
gold-set data-quality bug) moved to `../discovery/open-questions.md`** (2026-07-14, part of
promoting Discovery to its own study). Only the general-engine items stay here.

...
|---|---|---|
| [LitDiscover](litdiscover/) | Paper **desk-rejected by IP&M** (2026-07-07) — redo planned; engine on PyPI but **out of date** (v2.0.0 has a known bug, fixed locally 2026-07-07, not yet republished). Discovery phase promoted to its own study (`discovery/`, 2026-07-14) around a corrected end-to-end evaluation design | Information Processing & Management (redo) |
...
live files. Two phases have moved out to their own study directories (2026-07-14) since they're
no longer just "one implementation detail of the LitDiscover engine": Discovery
(`discovery/`, see below — promoted the same day §4.0's end-to-end redesign turned it into a
rigorous investigation in its own right) and Synthesis's corpus-structuring protocols
(`synthesis/`, see further below).**

| File | Purpose |
|---|---|
| [litdiscover/research-roadmap.md](litdiscover/research-roadmap.md) | **Supersedes `background.md` (2026-07-14).** Cross-stage roadmap (Discovery/Extraction/Synthesis) — current bets, benchmark design; discovery detail lives in its own study now (see the Discovery section below) |
| [../litdiscover/litdiscover/discovery/README.md](../litdiscover/litdiscover/discovery/README.md) | New 2026-07-14, lives next to the code it documents rather than in the wiki. Reference doc (not research framing) for how `litdiscover/discovery/` actually works post-refactor: module map, the `OperatorResult` contract, all 7 operators, the `GraphSource` protocol/`S2Source`/`ClosedCorpusSource`, rate limiting + budget accounting, and what the CLI actually calls vs. what's research-only |
...
| [litdiscover/open-questions.md](litdiscover/open-questions.md) | Live: open items, engine feature requests (discovery/screening-specific items moved to `discovery/open-questions.md`) |
| [litdiscover/deep-dives.md](litdiscover/deep-dives.md) | Live: source doc — 22 full 6-field method deep-dives + Methods/Evaluation-Methods tables, mined from the 366-paper corpus beyond the original 7, plus a 5-entry verification cohort (SWIFT-Review, RobotSearch, ASReview, Elicit, ResearchRabbit), plus SurveyLens (added 2026-07-14, found while researching the eval-standard question below) |

---

## Discovery

**Promoted 2026-07-14 from `litdiscover/phase-discovery-roadmap.md` to its own study** — the
same session that added a methodological correction (§4.0 in `roadmap.md`) turned this from "one
implementation detail of the LitDiscover engine" into a rigorous, defensible investigation
worth its own directory: isolated discovery-only or screening-only recall/precision numbers
aren't defensible on their own (precision's denominator is discovery-dependent — proven by this
project's own finding that a 73-100% recall headline implied 0.03-0.45% precision once anyone
checked), so the actual headline metric has to be **end-to-end recall/precision of the real
`included` set** against a survey's true bibliography, with the existing operator-level
ablation/marginal-contribution work demoted to *diagnostic decomposition*.

| File | Purpose |
|---|---|
| [discovery/roadmap.md](discovery/roadmap.md) | **Start here.** §1 implementation audit (current engine, the eval-code's three tiers of production-integration), §2 prior-art survey (27 methods, by discovery mechanism family), §3 the two-mechanism bet, **§4.0 — the methodological principle + primary end-to-end experiment design + "ironclad/defensible" checklist (new 2026-07-14)**, §4.1-§4.11 — operator-level ablation/ordering/budget work (now diagnostic-only, not the headline), §5 simulation-vs-production gap, §7 open decisions (step 0 = build the §4.0 harness, ahead of anything else) |
| [discovery/open-questions.md](discovery/open-questions.md) | Live: discovery/screening-specific open items pulled from `litdiscover/open-questions.md` — the pre-submission recall figures (Q1 now flagged stale against §4.0), the Groq screening-backend-fallback gap, and the gold-set data-quality bug (found + fixed 2026-07-14, recall numbers still need recomputing) |
| [discovery/background/corpus-curation-prior-art.md](discovery/background/corpus-curation-prior-art.md) | Companion to `deep-dives.md`, re-cut around one question: how does each surveyed system discover *and screen* its corpus? Comparison table (discovery mechanism / screening mechanism / stopping criterion / screening validated?) across all 27+5 methods, grouped analysis, and a "what this means for LitDiscover" section — this is also where the mature-eval-standard finding lives (CLEF TAR/SYNERGY is mature for screening-in-isolation, decades-old relative-recall methodology exists for discovery-in-isolation, but nobody validates the two *together* against a citation-traversal-fed queue — the exact gap §4.0 closes) |
...
| [synthesis/background/lineages/](synthesis/background/lineages/) | LLM-text-native track, **complete as a control condition** — three methods run over the same 27-paper corpus; still LitDiscover's own related-work-drafting source, grouped here because it's a disjoint corpus-structuring protocol, not because it stopped serving that role |
| [synthesis/background/reference-implementation-survey.md](synthesis/background/reference-implementation-survey.md) | Code-grounded (not paper-text) audit of 14 cloned reference systems: embedding/clustering choices, synthesis-mechanism deep-dives in `deep-dives.md` style, paper-vs-code fidelity gaps (SurveyX's "attribute forest" doesn't exist in code, SurveyGen's re-ranking formula doesn't match its stated coefficients, InteractiveSurvey's clustering variable is named `hdbscan_model` but instantiates `AgglomerativeClustering`), plus each system's paper-reported eval/results |
...
**`synthesis/background/lineages/` — three LLM-text-native corpus-structuring methods:**

| File | Method | Finding |
|---|---|---|
| [similarity-cluster.md](synthesis/background/lineages/similarity-cluster.md) | **Deprecated 2026-07-13** — thematic clustering, kept unedited as the control condition in `lineage-comparison.md`, not a drafting source | Only 12/32 real citation edges represented; 3 drawn edges have no textual support |
| [explicit-citation-graph.md](synthesis/background/lineages/explicit-citation-graph.md) | O(n) bottom-up — read each paper once, extract only what it explicitly states about others; connected-components analysis (union-find) on the resulting graph | 32 confirmed edges; 19-paper giant component, 2-paper satellite, isolates — ground truth for the other two methods |
| [implicit-pairwise-analysis.md](synthesis/background/lineages/implicit-pairwise-analysis.md) | O(n²)-ish content-matching — date-ordered pairwise check of named limitations vs. later mechanisms | 10 new uncited-but-real edges (likely undercounted); unioned with the 32 explicit edges, 2 previously fully-isolated papers (ResearchRabbit, Scholar Augment) get pulled into the field's main connected structure |
| [lineage-comparison.md](synthesis/background/lineages/lineage-comparison.md) | Worked example — ProfOlaf drawn all three ways | 6 real relationships total; no single method found more than half |
...
— via disjoint subroutines. `lineages/` operates on paper *text* (LLM-extracted
...
and manifold/spectral embedding (NST/UMAP/SG-t-SNE). `lineages/` and Q-SYNTH are both
...
**Discovery — promoted to its own study, Experiment 1 redesigned around end-to-end evaluation (2026-07-14, session 41):** Research into whether the field has a mature eval standard for discovery/screening (see `discovery/background/corpus-curation-prior-art.md`) surfaced a real methodological flaw in this project's own Experiment 1: isolated discovery-only or screening-only recall/precision numbers aren't defensible, since precision's denominator is discovery-dependent — the same fact that made the original system's 73-100% recall headline imply 0.03-0.45% precision once anyone checked (session 38's finding, below). `discovery/roadmap.md` §4.0 (new) defines the corrected primary experiment — end-to-end recall/precision of the actual `included` set against real survey bibliographies, budget-capped, funnel-reported — and demotes the existing operator-ablation work (§4.1-§4.11, the "Experiment 1" the two paragraphs below describe) to diagnostic-decomposition status: real, reusable, but not the headline claim anymore. `wiki/discovery/` is now its own top-level study (see the Discovery section above), with `discovery/open-questions.md` holding the discovery/screening-specific open items pulled from `litdiscover/open-questions.md`. **Next:** build the §4.0 end-to-end harness (§7 step 0) — supersedes recomputing the stale isolated-recall numbers below in isolation.

**LitDiscover — discovery unified across closed-corpus/live-S2, canonical result promoted (2026-07-14, session 39):** Built `litdiscover/discovery/graph_source.py` (`GraphSource`/`S2Source`/`ClosedCorpusSource`) so `backward_traversal_operator`/`forward_traversal_operator`/`pareto_hub_threshold`/`author_expansion_operator`/`venue_expansion_operator` run identically against live S2 or the closed APS corpus — 259/259 tests passing, all 227 pre-existing tests unmodified. Used it to migrate all 6 closed-corpus eval/sweep scripts + a new `07_operator_benchmark.py`, and `live-survey-eval/09`; deduplicated `10`/`11`/`12` into a shared `_shared.py`. **`eval/04b` (the paper's canonical cold-start result) was actually re-run and promoted**: a real filter-design flaw in the old Pareto implementation (discarded candidates by their own out-degree, not the frontier's) was found and fixed — full 54-condition comparison shows mean recall 93.5%→99.6%, mean corpus size 3.1x smaller. This leaves `05_miss_analysis.py`'s canonical condition with zero misses, so `06_publication_figures.py`'s Fig 7 has nothing to plot — paused, not resolved, pending a paper-facing decision. `eval/03`/`sweep/08`/`live-survey-eval/09` were migrated but deliberately not run to completion (real time/API cost at scale, a user-directed call, see `wiki/discovery/roadmap.md` §1.4). `.mat` file's author data confirmed technically inaccessible (MCOS-encoded `string` type, `mat73` explicitly rejects it) — author/venue-expansion benchmarking against the closed corpus stays S2-only.
...
**LitDiscover wiki:** `wiki/litdiscover/` now has `research-roadmap.md` (new 2026-07-14, superseding `background.md`) and `decisions.md`/`open-questions.md`/`deep-dives.md` (live, updated in place). `phase-representation-roadmap.md` and `lineages/` (three parallel corpus-structuring methods, 2026-07-13) moved to `wiki/synthesis/`; `phase-discovery-roadmap.md` and `corpus-curation-prior-art.md` moved to `wiki/discovery/` (as `roadmap.md`/`background/corpus-curation-prior-art.md`) — both the same day, both now their own top-level studies, see the Discovery/Synthesis sections above. Flagged, still not decided: RLS is disabled on all 6 tables in the `litreview-v2` Supabase project (anon key has full read/write).

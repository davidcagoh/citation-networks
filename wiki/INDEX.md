...
**LitDiscover (solo):** ❌ Desk-rejected by IP&M (2026-07-07) for missing SOTA/LLM baselines and current-year references. **Actively being redone (2026-07-09):** `automated-lit-review-methodology` reset and re-seeded with 8 fresh SOTA/2025-2026 anchors, traversed, prefiltered, and hand-eyeballed (366 included / 344 excluded / 147 still pending) directly against the rejection's stated gaps. Not yet re-extracted/re-synthesized — blocked on the Gemini spend cap (check ai.studio/spend) and another `traverse` cycle from the enlarged included set.

**LitDiscover the engine:** ✅ **Reworked to staged-by-default, autopilot opt-in** (2026-07-09) — `run` now performs one traversal cycle and stops by default; LLM screening only fires via deliberate `screen`/`mark`/`prefilter` calls, never automatically. New commands: `traverse`, `screen`, `prefilter`, `mark`, `related-work-mine`. `watchdog.py` and its unattended-rotation projects retired. 174 tests passing, merged to `main` (not yet republished to PyPI — still v2.0.0). Two real bugs found and fixed via the redo run above (related-work section-heading false-positive; prefilter term-derivation leak). Still not republished to PyPI. Still needed: bump + republish, and manually tear down the old host-side launchd job.
